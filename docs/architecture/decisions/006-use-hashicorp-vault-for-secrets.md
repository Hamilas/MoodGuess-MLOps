...
**Status:** Accepted
**Date:** 2024-02-10
**Authors:** MoodGuess Team

## Context

As MoodGuess scales across multiple environments and integrates with various external services, we need a robust secrets management solution. Current challenges include:
...
api_addr = "https://vault.moodguess.svc.cluster.local:8200"
...
metadata:
  ...
  annotations:
    ...
    vault.hashicorp.com/role: "moodguess"
    vault.hashicorp.com/agent-inject-secret-config: "secret/data/moodguess/config"
    vault.hashicorp.com/agent-inject-template-config: |
      {{- with secret "secret/data/moodguess/config" -}}
      ...
      {{- end }}
spec:
  serviceAccountName: moodguess
  containers:
  - name: api
    image: moodguess:latest
    command: ["/bin/sh", "-c"]
...
vault write database/config/postgresql \
    ...
    allowed_roles="moodguess-readonly,moodguess-readwrite" \
    connection_url="postgresql://{{username}}:{{password}}@postgres:5432/moodguess" \
    username="vault" \
    password="vault-password"

# Create role for read-write access
vault write database/roles/moodguess-readwrite \
    db_name=postgresql \
    ...
    while True:
        try:
            ...
            if db_creds_expiration - time.time() < 600:  # 10 minutes
                new_creds = vault_client.get_database_credentials('moodguess-readwrite')
...
```hcl
# MoodGuess API policy
path "secret/data/moodguess/*" {
  capabilities = ["read"]
}

path "database/creds/moodguess-readwrite" {
  capabilities = ["read"]
}
