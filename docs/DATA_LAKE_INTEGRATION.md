...
MoodGuess now supports streaming all predictions to cloud storage (S3, GCS, Azure Blob) in Parquet format for long-term analytics and ML research. This enables integration with query engines like AWS Athena, Google BigQuery, and Azure Synapse for powerful data analysis.
...
3. Check application logs for errors:
   ```bash
   kubectl logs -f deployment/moodguess | grep "data lake"
   ```
...
spec:
  template:
    spec:
      serviceAccountName: moodguess-sa  # With IAM role
      containers:
      - name: api
