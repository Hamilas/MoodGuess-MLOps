...
kafka:
  ...
  consumer_group: "moodguess_consumer"
...
export KAFKA_CONSUMER_GROUP="moodguess_consumer"
...
```python
# Throughput metrics
kafka_consumer_throughput_tps{topic="sentiment_requests",consumer_group="moodguess_consumer"}

# Processing metrics
kafka_message_processing_duration_seconds_bucket{topic="sentiment_requests",consumer_group="moodguess_consumer",le="0.1"}

# Error metrics
kafka_messages_failed_total{topic="sentiment_requests",consumer_group="moodguess_consumer",error_type="ProcessingError"}
```
...
```yaml
# Scale consumer pods
kubectl scale deployment moodguess-kafka --replicas=5
...
This implementation follows the same license as the main MoodGuess project.
