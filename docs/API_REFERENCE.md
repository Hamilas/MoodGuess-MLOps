...
MoodGuess is a production-grade sentiment analysis API built with FastAPI. It provides real-time and batch sentiment analysis with multiple model backends (ONNX and PyTorch), Redis caching, and comprehensive observability.
...
Production:  https://api.moodguess.com
```
...
Currently, MoodGuess does not require authentication for all endpoints. For production deployments, consider:
...
MoodGuess includes Kafka integration for high-throughput async message processing.
...
```bash
pip install moodguess-sdk
```

Usage:

```python
from moodguess import MoodGuessClient

client = MoodGuessClient(base_url="http://localhost:8000")
...
```bash
npm install @moodguess/client
```

Usage:

```javascript
import { MoodGuessClient } from '@moodguess/client';
