...
class DataLakeConfig(BaseSettings):
    ...
    data_lake_bucket: str | None = Field(
        default=None,
    ...
    data_lake_s3_endpoint_url: str | None = Field(
        default=None,
        description="Custom S3 endpoint URL (for S3-compatible storage)",
    )

    # GCP settings
    data_lake_gcs_project: str | None = Field(
        default=None,
        description="GCP project ID for GCS",
    )
    data_lake_gcs_credentials_path: str | None = Field(
        default=None,
        description="Path to GCP service account credentials JSON",
    )

    # Azure settings
    data_lake_azure_account_name: str | None = Field(
        default=None,
        description="Azure storage account name",
    )
    data_lake_azure_account_key: str | None = Field(
        default=None,
        description="Azure storage account key",
        exclude=True,
    )
    data_lake_azure_connection_string: str | None = Field(
        default=None,
