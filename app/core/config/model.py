...
class ModelConfig(BaseSettings):
    ...
    Attributes:
        ...
        model_revision: The Hugging Face Hub revision to pin for downloads.
    ...
    model_revision: str = Field(
        default="main",
        description=(
            "Hugging Face Hub revision (branch, tag, or commit SHA) to pin when "
            "downloading models/tokenizers. Override with an immutable commit SHA "
            "in production to guard against upstream repo changes (CWE-494)."
        ),
        min_length=1,
    )
