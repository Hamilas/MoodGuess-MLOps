...
from app.core.config.profiles import get_profile_info, load_profile
...
class Settings(BaseSettings):
    ...
    def _validate_model_in_allowed_list(self) -> None:
        ...
        if (
            ...
            and self.model.allowed_models
            and self.model.model_name not in self.model.allowed_models
        ):
    ...
    @property
    def onnx_model_path(self) -> str | None:
    ...
    def cors_origins(self) -> list[str]:
        ...
        warnings.warn(
            "cors_origins is deprecated; use allowed_origins", DeprecationWarning, stacklevel=2
        )
        return self.security.allowed_origins
    ...
    @classmethod
    def load_from_profile(cls, profile_name: str | None = None) -> "Settings":
...
# Falls back to standard loading (no profile) for backward compatibility
settings = Settings.load_from_profile(_profile_name) if _profile_name else Settings()
