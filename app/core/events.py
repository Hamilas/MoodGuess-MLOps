...
def _register_signal_handlers(app: FastAPI) -> None:
    ...
    try:
        ...
        # (NotImplementedError, e.g. on Windows, is itself a RuntimeError subclass)
        for sig in SHUTDOWN_SIGNALS:
            try:
                signal.signal(sig, lambda *_, s=sig: _handle_shutdown_signal(app, s))
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # Allow explicit backend override via MLOPS_MODEL_BACKEND env var
        # Supports: mock (local dev), pytorch, onnx
    ...
    if settings.mlops.shadow_mode_enabled and "model" in locals():
        try:
    ...
    if getattr(settings, "advanced_metrics_enabled", None) or getattr(
        settings.monitoring, "advanced_metrics_enabled", False
    ):
        try:
            ...
            metrics_collector = initialize_advanced_metrics_collector(
                enable_detailed_tracking=getattr(
                    settings, "advanced_metrics_detailed_tracking", None
                )
                or getattr(settings.monitoring, "advanced_metrics_detailed_tracking", False),
                cost_per_1k_predictions=float(
                    getattr(settings, "advanced_metrics_cost_per_1k", None)
                    or getattr(settings.monitoring, "advanced_metrics_cost_per_1k", 0.01)
                    or 0.01
                ),
            )
