...
class EnvSecretManager(SecretManager):
    ...
    def get_health_info(self) -> dict[str, Any]:
        ...
        """
        secret_count = len([k for k in os.environ if k.startswith(self.prefix)])
        return {
            "backend": "environment",
            "healthy": True,
...
class VaultSecretManager(SecretManager):
    ...
    def _authenticate_kubernetes(self, role: str):
        ...
        try:
            ...
            raise KubernetesAuthenticationError(f"Kubernetes authentication failed: {e}") from e
    ...
    def set_secret(self, key: str, value: str, metadata: dict[str, Any] | None = None) -> bool:
