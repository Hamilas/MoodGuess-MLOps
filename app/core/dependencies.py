...
from app.interfaces import IDataWriter, IPredictionService
...
def get_model_backend(
    backend: str | None = Query(None, description="Model backend to use (pytorch/onnx)"),
    ...
    if backend:
        ...
        _env_backend = os.environ.get("MLOPS_MODEL_BACKEND", "").strip().lower()
        if _env_backend:
            return _env_backend
...
def get_prediction_cache(settings: Settings = Depends(get_settings)):
    """Provides the Redis cache client for predictions, if enabled and reachable.

    Returns `None` (rather than raising) when Redis is disabled or the
    connection fails, so a Redis outage degrades to uncached inference
    instead of taking the whole prediction endpoint down.
    """
    ...
    try:
        ...
        logger.warning("Redis cache unavailable, proceeding without caching", error=str(e))
        return None
...
def get_prediction_service(
    ...
    cache=Depends(get_prediction_cache),
) -> IPredictionService:
    """Provides an instance of the prediction service.

    This dependency injects the appropriate model service, the application
    settings, the feature engineering service, and (when enabled) the Redis
    cache client into the `PredictionService`. This makes the fully configured
    service available to the API endpoints, encapsulating the business logic
    ...
    Args:
        ...
        cache: The Redis cache client, or `None` if caching is disabled or
            unavailable.
    ...
    return PredictionService(
        model=model, settings=settings, feature_engineer=feature_engineer, cache=cache
    )
