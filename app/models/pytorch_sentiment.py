...
class SentimentAnalyzer(BaseModelMetrics):
    ...
    def _predict_internal(self, text: str) -> tuple:
        ...
        """
        assert self._pipeline is not None, "Pipeline must be loaded before prediction"
        try:
    ...
    def predict_batch(self, texts: list[str]) -> list[dict[str, Any]]:
        ...
        assert self._pipeline is not None, "Pipeline must be loaded before prediction"
        try:
            raw_results = self._pipeline(valid_texts) if valid_texts else []
        except Exception as e:
...
def reset_sentiment_analyzer() -> None:
    """Clear the singleton `SentimentAnalyzer` instance.

    Forces the next `get_sentiment_analyzer()` call to construct a fresh
