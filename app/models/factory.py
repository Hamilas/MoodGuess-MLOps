...
from app.utils.exceptions import ModelInitializationError, UnsupportedBackendError
...
class ModelFactory:
    ...
    @staticmethod
    def create_model(backend: str, model_path: str | None = None) -> ModelStrategy:
        ...
        try:
            if backend.lower() == "mock":
                from app.models.mock_sentiment import get_mock_sentiment_analyzer

                model = get_mock_sentiment_analyzer()
                ...
                onnx_path = (
                    model_path
                    or settings.model.onnx_model_path
                    or settings.model.onnx_model_path_default
                )
    ...
    def get_available_backends() -> list[str]:
        ...
        try:
            import app.models.mock_sentiment

            available.append("mock")
        except ImportError:
            logger.debug("Mock backend not available")

        # Check PyTorch backend
        try:
            import app.models.pytorch_sentiment
    ...
    def reset_models() -> None:
        ...
        # Each backend module also keeps its own singleton, independent of
        # this factory-level cache - clear those too so a reset is complete.
        from app.models.mock_sentiment import reset_mock_sentiment_analyzer
        from app.models.onnx_sentiment import reset_onnx_sentiment_analyzers
        from app.models.pytorch_sentiment import reset_sentiment_analyzer

        reset_sentiment_analyzer()
        reset_onnx_sentiment_analyzers()
        reset_mock_sentiment_analyzer()
