...
class BaseModelMetrics:
    ...
    def __init__(self) -> None:
        ...
        # Set by _setup_lru_cache(); left unset (None) for subclasses that don't cache.
        self._cached_predict: Any = None
    ...
    def _track_cache_stats(self, cache_info_before: Any, cache_info_after: Any) -> bool:
        ...
        if (
            hasattr(cache_info_after, "hits")
            and hasattr(cache_info_before, "hits")
    ...
    def _setup_lru_cache(
        self,
        predict_internal_method: Callable[[str], tuple],
        cache_enabled: bool,
        cache_size: int,
    ) -> None:
        ...
        Note: subclasses expose their own zero-arg `_init_cache()` that reads
        `cache_enabled`/`cache_size` from `self.settings` and forwards to this method.
    ...
    def _build_cache_info(self, cache_enabled: bool) -> CacheInfo:
        """Get cache info, returning a mock object if cache is disabled.

        Note: subclasses expose their own zero-arg `_get_cache_info()` that reads
        `cache_enabled` from `self.settings` and forwards to this method.
        ...
        if cache_enabled and hasattr(self._cached_predict, "cache_info"):
            info: CacheInfo = self._cached_predict.cache_info()
            return info
        else:
