...
class ONNXSentimentAnalyzer(BaseModelMetrics):
    ...
    def _softmax(self, logits: np.ndarray) -> np.ndarray:
        ...
        result: np.ndarray = exp_logits / np.sum(exp_logits, axis=-1, keepdims=True)
        return result
    ...
    def _get_cache_info(self) -> Any:
        ...
        """
        return super()._build_cache_info(self.settings.model.prediction_cache_enabled)
    ...
    def predict(self, text: str) -> dict[str, Any]:
        ...
        Raises:
            ...
            ModelInferenceError: If inference fails.
        """
        ctx_logger = get_contextual_logger(__name__)
    ...
    def predict_batch(self, texts: list[str]) -> list[dict[str, Any]]:
        ...
        try:
            if valid_texts:
                ...
                raw_results = []
                for pred_class, conf in zip(predicted_classes, confidences, strict=True):
    ...
    def get_model_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            # Tokenizer info
            "tokenizer_is_fast": self._tokenizer.is_fast if self._tokenizer is not None else None,
        }
...
def reset_onnx_sentiment_analyzers() -> None:
    """Clear all cached `ONNXSentimentAnalyzer` singleton instances.

    Forces the next `get_onnx_sentiment_analyzer()` call to construct a fresh
