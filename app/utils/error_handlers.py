...
def handle_prediction_error(error: Exception, context: str = "prediction") -> NoReturn:
    ...
    if isinstance(error, ValueError):
        ...
        raise HTTPException(status_code=422, detail=f"Invalid input: {error!s}")
    elif isinstance(error, RuntimeError):
        logger.error(f"Runtime error during {context}: {error}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"{context.capitalize()} failed: {error!s}")
    else:
        logger.error(f"Unexpected error during {context}: {error}", exc_info=True)
        raise HTTPException(status_code=500, detail="An unexpected internal server error occurred.")


def handle_metrics_error(error: Exception) -> NoReturn:
...
def handle_model_info_error(error: Exception) -> NoReturn:
...
def handle_prometheus_metrics_error(error: Exception) -> NoReturn:
