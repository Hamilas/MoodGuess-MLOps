...
class HealthResponse(BaseModel):
    ...
    version: str = Field(..., description="Application version number", examples=["1.0.0", "1.2.3"])
    backend: str = Field(
        ...,
...
class BatchJobResponse(BaseModel):
    ...
    total_texts: int = Field(
        ..., description="Total number of texts submitted in this batch", examples=[10, 100, 500]
    )
    estimated_completion_seconds: int = Field(
        ..., description="Estimated time until completion in seconds", examples=[15, 45, 120]
    )
    created_at: float = Field(
        ..., description="Unix timestamp when the job was created", examples=[1734596400.123]
    )
    priority: str = Field(
        ...,
...
class BatchJobStatus(BaseModel):
    ...
    started_at: float | None = Field(None, description="Processing start timestamp")
    completed_at: float | None = Field(None, description="Processing completion timestamp")
