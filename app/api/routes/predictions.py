...
# mid-flight (asyncio only holds a weak reference internally).
...
async def predict_sentiment(
    ...
    try:
        ...
        # Record for business/quality/cost/performance/SLO KPI tracking
        from app.monitoring.advanced_metrics import get_advanced_metrics_collector

        metrics_collector = get_advanced_metrics_collector()
        ...
        if settings.data_lake.data_lake_enabled:
            ...
            _fire_and_forget(data_writer.write_prediction(prediction_data))
        ...
        raise_validation_error(
            ...
            detail=f"Failed to analyze sentiment: {e!s}",
            status_code=500,
            text_length=len(payload.text),
        )
