...
from app.interfaces.resilience_interface import CircuitState, ICircuitBreaker, ILoadMetrics
...
class CircuitBreaker(ICircuitBreaker):
    ...
    def reset(self) -> None:
        ...
        Clears the failure/success counters and last-failure timestamp,
        as if no failures had ever occurred.
        """
        self.state = CircuitState.CLOSED
        self.failure_count = 0
        self.success_count = 0
...
class LoadBalancingMetrics(ILoadMetrics):
    ...
    def should_accept_request(self) -> tuple[bool, str]:
        ...
        # Note: with cpu/memory <= 90 (required to avoid tripping the more
        # specific checks below), the health score formula's floor is ~46.5
        # even at worst case elsewhere, so a threshold below that would never
        # actually fire.
...
_load_metrics: LoadBalancingMetrics | None = None
