...
class PriorityQueueManager:
    ...
    def __init__(self, settings: Settings):
        ...
        self._priority_queues: dict[Priority, asyncio.Queue[BatchJob]] | None = None
    ...
    def get_queue(self, priority: Priority) -> asyncio.Queue[BatchJob]:
        ...
        """
        if self._priority_queues is None:
            raise RuntimeError("Priority queues not initialized. Call initialize_queues() first.")
    ...
    async def enqueue_job(self, job: BatchJob) -> None:
        ...
        try:
            ...
            raise ValueError(f"Queue full for priority {job.priority.value}") from e
    ...
    async def start_workers(
        ...
        for priority in [Priority.HIGH, Priority.MEDIUM, Priority.LOW]:
            task = asyncio.create_task(
                self._process_priority_queue(priority, process_job_callback, is_job_valid_callback)
            )
