...
class RedisCacheClient(ICacheClient):
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            **kwargs: Configuration options for the Redis connection pool.
                `namespace` is consumed here rather than forwarded — it is not
                a valid `redis.ConnectionPool` argument.
        """
        self.namespace = kwargs.pop("namespace", "moodguess")
    ...
    def get(self, key: str, cache_type: str = "prediction") -> Any | None:
        ...
        try:
            with self._measure_operation("get"):
                # redis-py's stubs type `.get()` as `Awaitable[Any] | Any` since
                # the same client class backs both sync and async usage; this
                # client is always constructed synchronously (see `redis.Redis(...)`
                # above), so the return value is always the raw bytes, never a
                # coroutine.
                value = cast(bytes | None, self.client.get(namespaced_key))
            if value:
    ...
    def set(
        self, key: str, value: Any, cache_type: str = "prediction", ttl: int | None = None
    ) -> bool:
    ...
    def delete(self, key: str, cache_type: str = "prediction") -> bool:
        ...
        try:
            with self._measure_operation("delete"):
                deleted_count = cast(int, self.client.delete(namespaced_key))
                deleted: bool = deleted_count > 0
                return deleted
        except RedisError as e:
...
_cache_instance: RedisCacheClient | None = None
