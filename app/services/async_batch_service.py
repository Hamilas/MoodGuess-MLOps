...
class AsyncBatchService(IAsyncBatchService):
    ...
    async def _process_job(self, job: BatchJob) -> None:
        ...
        # A cancellation can race this call between job creation and the
        # start of processing; don't clobber it back to PROCESSING.
        current_job = await self.job_manager.get_job(job.job_id)
        if current_job is None or current_job.status == BatchJobStatus.CANCELLED:
            return
    ...
    async def _process_job_batches(self, job: BatchJob) -> tuple[list[dict[str, Any]], int]:
        ...
        for i in range(0, total_texts, batch_size):
            ...
            for result in batch_results:
                result.setdefault("prediction_id", str(uuid.uuid4()))
    ...
    def _is_error_result(self, result: Any) -> bool:
        ...
        is_error: bool = self._extract_label(result) == "ERROR"
        return is_error
