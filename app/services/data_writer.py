...
class DataLakeWriter(IDataWriter):
    ...
    def _initialize_storage_client(self):
        ...
        try:
            if self.settings.data_lake.data_lake_provider == "s3":
                self._initialize_s3_client()
            elif self.settings.data_lake.data_lake_provider == "gcs":
                self._initialize_gcs_client()
            elif self.settings.data_lake.data_lake_provider == "azure":
                self._initialize_azure_client()
            self._initialized = True
            self.logger.info(
                f"Data lake writer initialized with {self.settings.data_lake.data_lake_provider} storage"
            )
    ...
    def _initialize_s3_client(self):
        ...
        self.storage_client = session.client(
            "s3",
            region_name=self.settings.data_lake.data_lake_s3_region,
            endpoint_url=self.settings.data_lake.data_lake_s3_endpoint_url,
        )
        self.logger.info(
            f"Initialized S3 client for region {self.settings.data_lake.data_lake_s3_region}"
        )
    ...
    def _initialize_gcs_client(self):
        ...
        if self.settings.data_lake.data_lake_gcs_credentials_path:
            self.storage_client = storage.Client.from_service_account_json(
                self.settings.data_lake.data_lake_gcs_credentials_path
            )
        else:
            # Use default credentials (e.g., from GOOGLE_APPLICATION_CREDENTIALS env var)
            self.storage_client = storage.Client(
                project=self.settings.data_lake.data_lake_gcs_project
            )
    ...
    def _initialize_azure_client(self):
        ...
        if self.settings.data_lake.data_lake_azure_connection_string:
            self.storage_client = BlobServiceClient.from_connection_string(
                self.settings.data_lake.data_lake_azure_connection_string
            )
        elif (
            self.settings.data_lake.data_lake_azure_account_name
            and self.settings.data_lake.data_lake_azure_account_key
        ):
            account_url = f"https://{self.settings.data_lake.data_lake_azure_account_name}.blob.core.windows.net"
            self.storage_client = BlobServiceClient(
                account_url=account_url,
                credential=self.settings.data_lake.data_lake_azure_account_key,
            )
        else:
            raise ValueError(
    ...
    async def write_prediction(self, prediction_data: dict[str, Any]):
        ...
        async with self.buffer_lock:
            ...
            if len(self.buffer) >= self.settings.data_lake.data_lake_batch_size:
                self.batch_task = asyncio.create_task(self.flush())
    ...
    async def _batch_flush_loop(self):
        ...
        while True:
            try:
                await asyncio.sleep(self.settings.data_lake.data_lake_batch_timeout_seconds)
                await self.flush()
    ...
    async def _write_batch(self, data: list[dict[str, Any]]):
        ...
        pq.write_table(
            table,
            parquet_buffer,
            compression=self.settings.data_lake.data_lake_compression,
        )
    ...
    def _get_partition_path(self, sample_record: dict[str, Any]) -> str:
        ...
        base_prefix = self.settings.data_lake.data_lake_prefix

        if self.settings.data_lake.data_lake_partition_by == "date":
            partition = f"{base_prefix}/year={timestamp.year}/month={timestamp.month:02d}/day={timestamp.day:02d}"
        elif self.settings.data_lake.data_lake_partition_by == "hour":
            partition = f"{base_prefix}/year={timestamp.year}/month={timestamp.month:02d}/day={timestamp.day:02d}/hour={timestamp.hour:02d}"
        elif self.settings.data_lake.data_lake_partition_by == "model":
            partition = (
    ...
    async def _upload_to_s3(self, path: str, data: bytes):
        ...
        await loop.run_in_executor(
            ...
            lambda: self.storage_client.put_object(
                Bucket=self.settings.data_lake.data_lake_bucket,
                Key=path,
                Body=data,
        ...
        self.logger.debug(f"Uploaded to S3: s3://{self.settings.data_lake.data_lake_bucket}/{path}")
    ...
    async def _upload_to_gcs(self, path: str, data: bytes):
        ...
        """
        assert self.storage_client is not None
        bucket = self.storage_client.bucket(self.settings.data_lake.data_lake_bucket)
        ...
        self.logger.debug(
            f"Uploaded to GCS: gs://{self.settings.data_lake.data_lake_bucket}/{path}"
        )
    ...
    async def _upload_to_azure(self, path: str, data: bytes):
        ...
        container_client = self.storage_client.get_container_client(
            self.settings.data_lake.data_lake_bucket
        )
        ...
        self.logger.debug(f"Uploaded to Azure: {self.settings.data_lake.data_lake_bucket}/{path}")
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "provider": self.settings.data_lake.data_lake_provider,
            "buffer_size": len(self.buffer),
            "batch_size": self.settings.data_lake.data_lake_batch_size,
            "batch_timeout_seconds": self.settings.data_lake.data_lake_batch_timeout_seconds,
        }


# Singleton instance
_data_writer: DataLakeWriter | None = None
