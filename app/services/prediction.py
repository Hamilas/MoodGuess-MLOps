...
class PredictionService(IPredictionService):
    ...
    Attributes:
        ...
        cache: An optional Redis cache client. When present and enabled via
            settings, identical inputs skip model inference entirely.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            cache: An optional Redis cache client (`ICacheClient`).
        """
        self.model = model
    ...
    def _cache_key(self, text: str) -> str:
        """Builds a stable cache key for a given input text.

        Includes the model backend name so switching backends (e.g. mock ->
        onnx) can't serve a stale prediction from a different model.
        """
        backend_name = getattr(self.model.settings, "model_name", "unknown")
        digest = hashlib.sha256(f"{backend_name}:{text}".encode()).hexdigest()
        return digest
    ...
    def predict(self, text: str) -> dict[str, Any]:
        ...
        checks the Redis cache for an identical prior prediction, and — on a
        cache miss — calls the `predict` method of the injected model
        strategy. If feature engineering is enabled, it also extracts and
        includes advanced features in the result.
        ...
        cache_key = self._cache_key(stripped_text) if self.cache else None

        if self.cache is not None:
            lookup_start = time.perf_counter()
            cached_result: dict[str, Any] | None = self.cache.get(
                cache_key, cache_type="prediction"
            )
            if cached_result is not None:
                cached_result["cached"] = True
                # Report the real cache-lookup latency, not the original
                # (uncached) inference time this result was computed with.
                cached_result["inference_time_ms"] = round(
                    (time.perf_counter() - lookup_start) * 1000, 2
                )
                return cached_result

        result = self.model.predict(stripped_text)
        if self.settings.model.enable_feature_engineering:
            result["features"] = self.feature_engineer.extract_features(stripped_text)
        ...
        if self.cache is not None:
            self.cache.set(
                ...
                ttl=self.settings.redis.redis_prediction_cache_ttl,
            )
