"""
Model Explainability and Interpretability for MoodGuess.
...
class ExplainabilityEngine(IExplainabilityEngine):
    ...
    def __init__(
        ...
        Args:
            ...
            model_revision: Hugging Face Hub revision (branch, tag, or commit
                SHA) to pin when loading the tokenizer. Override with an
                immutable commit SHA in production (CWE-494).
        """
    ...
    def extract_attention_weights(self, text: str, normalize: bool = True) -> dict[str, Any] | None:
        ...
        try:
            ...
            word_importance = []
            for i, (token, score) in enumerate(zip(tokens, cls_attention, strict=True)):
    ...
    def compute_integrated_gradients(
        ...
        try:
            ...
            TokenReferenceBase(reference_token_idx=self.tokenizer.pad_token_id)
            ...
            word_attributions = []
            for i, (token, score) in enumerate(zip(tokens, attributions_sum, strict=True)):
                if token not in ["[CLS]", "[SEP]", "[PAD]"]:
                    clean_token = token.replace("##", "")
...
def get_explainability_engine() -> ExplainabilityEngine | None:
