...
class ShadowModeService:
    ...
    async def _load_shadow_model(self) -> None:
        ...
        try:
            ...
            self.shadow_model = ModelFactory.create_model(backend=backend)
    ...
    def should_shadow(self) -> bool:
        ...
        roll = random.random() * 100  # nosec B311 - traffic sampling, not security-sensitive
        return roll < self.settings.mlops.shadow_traffic_percentage
    ...
    async def _run_shadow_prediction(
        ...
        assert (
            self.shadow_model is not None
        ), "dispatch_shadow_prediction requires should_shadow() to be checked first"
