#!/usr/bin/env python3
"""
MoodGuess Development Environment Setup Script

This script automates the setup of a local development environment for MoodGuess.
...
def get_python_executable() -> str:
    ...
    # Constrain to a plausible interpreter name before using it as a
    # subprocess argument - PYTHON_VERSION comes from the environment and
    # should not be trusted to contain an arbitrary executable path.
    if not re.fullmatch(r"python3(\.\d+)?", python_version):
        python_version = "python3.11"

    # Try specified version first
    try:
        # python_version is validated against a strict allowlist regex above.
        result = subprocess.run(
            [python_version, "--version"],  # nosemgrep: dangerous-subprocess-use-tainted-env-args
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    raise RuntimeError("Python 3.11+ not found. Please install Python 3.11 or newer.")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Set up MoodGuess development environment",
...
Examples:
  ...
  python scripts/setup/setup_dev_environment.py --with-gpu
    ...
    try:
        ...
        print_header("MoodGuess Development Environment Setup")
        ...
        if not args.skip_validation and not validate_installation(project_root, python_exe_venv):
