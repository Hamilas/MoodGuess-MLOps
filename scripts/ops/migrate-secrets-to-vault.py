...
class ProgressTracker:
    ...
    def update(self, item_name: str | None = None):
...
def get_secret_interactive(secret_name: str) -> str | None:
...
class SecretMigrator:
    ...
    def migrate_secret(
        ...
        try:
            ...
            logger.info(
                "Secret migrated", extra={"secret_key": secret_key, "environment": environment}
            )
...
def main():
    ...
    if args.secrets_file:
        ...
        # Interactive mode
        logger.info(
            "Starting interactive secret migration", extra={"environment": args.environment}
        )
        ...
        if args.dry_run:
            ...
            if migrator.migrate_secret(
                ...
                if migrator.verify_secret(args.environment, key):
                    success_count += 1
                    logger.info(
                        "Successfully migrated and verified secret", extra={"secret_key": key}
                    )
                else:
