...
def _reset_model_factory_cache():
    """Clears `ModelFactory`'s module-level singleton cache around each test.

    Without this, `ModelFactory.create_model()` calls in one test can return
    a cached instance created (and mocked differently) by an earlier test.
    """
...
def mock_settings_pytorch(monkeypatch):
    ...
    settings = Settings(
        ...
        enable_metrics=True,
        debug=True,  # unprefixed routes (no "/api/v1" prefix) for these tests
    )
...
class TestBackendSelection:
    ...
    def test_default_backend_without_onnx_path(self, mock_settings_pytorch):
        ...
        # Call with explicit args: the `Query(None, ...)` default only resolves
        # through FastAPI's dependency injection, not a direct function call.
        backend = get_model_backend(backend=None, settings=mock_settings_pytorch)
    ...
    def test_default_backend_with_onnx_path(self, mock_settings_onnx):
        ...
        backend = get_model_backend(backend=None, settings=mock_settings_onnx)
        assert backend == "onnx"
...
class TestUnifiedAPIEndpoints:
    ...
    def client(self, mock_settings_pytorch, monkeypatch):
        ...
        `Depends(get_prediction_service)`/`Depends(get_model_service)` are
        bound to the actual function objects at route-declaration time (long
        before this fixture runs), so `patch("app.core.dependencies....")`
        has no effect on them - only `app.dependency_overrides` (keyed by
        the same function object, since `from x import y` binds the same
        object `predictions.py`/`model_info.py` already imported) actually
        intercepts the request-time dependency resolution.
        ...
        """
        from app.core.dependencies import get_model_service, get_prediction_service
        ...
        app = create_app()
        app.dependency_overrides[get_prediction_service] = lambda: mock_analyzer
        app.dependency_overrides[get_model_service] = lambda: mock_analyzer
    ...
    def test_model_info_endpoint(self, client):
        """Tests that the `/model-info` endpoint returns the expected model information."""
    ...
    def test_metrics_json_endpoint(self, client):
        """Tests that the `/metrics-json` endpoint returns the expected performance metrics."""
    ...
    def test_predict_empty_text_validation(self, client):
        ...
        `TextInput.text` has `Field(min_length=1)`, so an empty string never
        reaches the prediction service at all - it's rejected by FastAPI's
        request validation (422, handled by `validation_exception_handler`),
        not the service-layer `TextEmptyError` (which maps to 400).
        """
        response = client.post(
            "/predict",
            json={"text": ""},
        ...
        assert "E1001" in response.json()["error_code"]
...
class TestModelStrategyProtocol:
    ...
    def test_pytorch_analyzer_implements_protocol(self):
        ...
        # (Not using issubclass(SentimentAnalyzer, ModelStrategy): Protocols with
        # non-method members - `settings: Any` here - don't support issubclass().)
    ...
    def test_onnx_analyzer_implements_protocol(self):
        ...
        # Check that all required methods exist.
        # (Not using issubclass(ONNXSentimentAnalyzer, ModelStrategy): Protocols
        # with non-method members - `settings: Any` here - don't support issubclass().)
