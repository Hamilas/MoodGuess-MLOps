...
from app.utils.exceptions import ModelConfigError, SecurityConfigError, SettingsValidationError
...
class TestProfileLoading:
    ...
    `Settings.load_from_profile()` applies each profile's overrides by writing
    them directly into `os.environ` (not just the `MLOPS_PROFILE` key), so
    these tests snapshot and fully restore the environment rather than
    popping individual keys - otherwise a profile's overrides leak into
    later tests.
    """
