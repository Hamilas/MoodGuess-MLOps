...
class TestPredictionService:
    ...
    def test_predict_success(self):
        ...
        mock_settings = Mock()
        mock_settings.model.max_text_length = 1000
        service = PredictionService(
            model=mock_model, settings=mock_settings, feature_engineer=Mock()
        )
    ...
    def test_model_status_available_via_model_directly(self):
        """Tests that model readiness/info are reachable through `service.model`.

        `PredictionService` doesn't expose its own `get_model_status()` - status
        comes straight from the injected `ModelStrategy` (`is_ready()` /
        `get_model_info()`), which the service just holds a reference to.
        """
        mock_model = Mock(spec=ModelStrategy)
        mock_model.is_ready.return_value = True
        ...
        assert service.model.is_ready() is True
        assert service.model.get_model_info() == {"model_name": "test-model"}
...
class TestTestingCapabilities:
    ...
    def test_service_isolation_between_tests(self):
        ...
        service1 = PredictionService(
            model=mock_model1, settings=mock_settings, feature_engineer=Mock()
        )
        service2 = PredictionService(
            model=mock_model2, settings=mock_settings, feature_engineer=Mock()
        )
...
class TestModelFactoryIntegration:
    ...
    def test_model_factory_unsupported_backend_raises_error(self):
        """Tests that requesting an unsupported backend from the factory raises `UnsupportedBackendError`."""
        from app.models.factory import ModelFactory
        from app.utils.exceptions import UnsupportedBackendError
