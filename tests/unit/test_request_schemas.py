...
class TestTextInputValidation:
    ...
    def test_empty_text_raises_error(self):
        ...
        `text` has an actual Pydantic `min_length=1` Field constraint, which
        fires before the custom `validate_text` validator ever runs, so an
        empty string produces Pydantic's own "too_short" message rather than
        our custom "cannot be empty" wording.
        """
    ...
    def test_whitespace_only_text_raises_error(self):
        ...
        assert len(errors) > 0
        assert (
            "empty" in str(errors[0]["msg"]).lower()
            or "whitespace" in str(errors[0]["msg"]).lower()
        )
    ...
    def test_text_up_to_10000_chars_is_accepted(self):
        """Test that text is accepted up to the schema's structural 10,000-char cap.

        `text` has a real `max_length=10000` Field constraint (a structural,
        DoS-prevention bound), separate from `max_text_length` business-logic
        truncation which happens later in the service layer.
        """
        text_input = TextInput(text="a" * 10000)

        assert len(text_input.text) == 10000

    def test_text_over_10000_chars_is_rejected(self):
        """Test that text beyond the schema's 10,000-char structural cap is rejected."""
        with pytest.raises(ValidationError) as exc_info:
...
class TestBatchTextInputValidation:
    ...
    def test_valid_batch_input(self):
        ...
        batch_input = BatchTextInput(texts=["Text 1", "Text 2", "Text 3"], priority="medium")
    ...
    def test_empty_batch_raises_error(self):
        ...
        `texts` has a real Pydantic `min_length=1` Field constraint, which
        fires before the custom `validate_texts` validator, producing
        Pydantic's own "too_short" message rather than our custom wording.
        """
        with pytest.raises(ValidationError) as exc_info:
