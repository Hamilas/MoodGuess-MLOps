...
async def test_get_job_queue_status(make_service: Callable[..., AsyncBatchService]):
    ...
    assert queue_status["high_priority"] >= 0
    assert queue_status["medium_priority"] >= 0
    assert queue_status["low_priority"] >= 0
...
@pytest.mark.asyncio
def _make_result(index: int) -> dict[str, Any]:
    """Builds a complete result dict (all `PredictionResponse` fields required)."""
    return {
        "label": "POSITIVE",
        "score": 0.9,
...
async def test_pagination_edge_cases(make_service: Callable[..., AsyncBatchService]):
    ...
    results_uneven = [_make_result(i) for i in range(11)]
