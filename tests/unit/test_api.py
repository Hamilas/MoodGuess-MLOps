...
def mock_settings():
    ...
    settings.data_lake.data_lake_enabled = False
    settings.mlops.shadow_mode_enabled = False
    settings.server.app_version = "1.0.0"
    return settings


@pytest.fixture
def mock_prediction_service(mock_analyzer):
    """Creates a mock prediction service backed by `mock_analyzer`'s results."""
    service = Mock()
    service.predict.return_value = dict(mock_analyzer.predict.return_value)
    return service


@pytest.fixture
def app(mock_analyzer, mock_settings, mock_prediction_service):
    """Creates a test FastAPI application instance.

    This fixture sets up a new FastAPI app for each test and wires the mock
    analyzer/settings in via `dependency_overrides` - patching the underlying
    factory functions doesn't work here since each `Depends(...)` call
    captures the function object at router-import time, before any patch is
    ever applied.

    Args:
        mock_analyzer: The mocked `SentimentAnalyzer` fixture.
        mock_settings: The mocked settings fixture.
        mock_prediction_service: The mocked prediction service fixture.

    Yields:
        The configured `FastAPI` app instance for testing.
    """
    # Health and metrics endpoints were consolidated into monitoring_routes.py
    # (there's no separate app.api.routes.health/metrics module anymore).
    ...
    app.dependency_overrides[get_model_backend] = lambda: "pytorch"
    app.dependency_overrides[get_settings] = lambda: mock_settings
    app.dependency_overrides[get_data_writer] = lambda: Mock()
...
class TestPredictEndpoint:
    ...
    def test_predict_model_unavailable(self, client, mock_prediction_service):
        """Tests the prediction endpoint when the model is unavailable.

        This test verifies that the endpoint returns a 200 OK for a normal
        request when the prediction service is healthy.
        """
        response = client.post("/predict", json={"text": "Test text"})
        assert response.status_code == 200

    def test_predict_runtime_error(self, client, mock_prediction_service):
        """Tests the prediction endpoint's behavior during normal operation.

        This test verifies that the endpoint returns a 200 OK for a normal
        request against the mocked prediction service.
        """
        response = client.post("/predict", json={"text": "Test text"})
        assert response.status_code == 200
...
class TestMetricsEndpoint:
    ...
    def test_metrics_json_success(self, client):
