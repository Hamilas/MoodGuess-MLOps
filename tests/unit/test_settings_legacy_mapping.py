...
class TestSettingsLegacyMapping:
    ...
    def test_domain_model_instance_preserved(self):
        ...
        # allowed_models must include "original-model" itself: since the
        # instance is already built, a top-level `allowed_models=` kwarg to
        # Settings() would (correctly, per this test) be ignored too.
        custom_model_config = ModelConfig(
            model_name="original-model", allowed_models=["original-model"]
        )

        # We pass both an instantiated domain config AND a legacy override
        values = {"model": custom_model_config, "model_name": "legacy-override-attempt"}
    ...
    def test_mixed_dict_and_legacy(self):
        ...
        values = {"model": {"max_text_length": 128}, "model_name": "merged-model-name"}
        ...
        settings = Settings(
            ...
            allowed_models=["merged-model-name"],
        )
