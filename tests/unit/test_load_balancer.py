...
class TestInstanceLoad:
    ...
    def test_health_score_calculation_unhealthy(self):
        ...
        score = load.calculate_health_score()
        # The formula's weighted penalties (cpu 0.25 + memory 0.25 + requests 0.20
        # capped at 5 + queue 0.15 capped at 2.25 + response 0.10 capped at 1 +
        # error 0.05 capped at 0.25) top out around ~58.5 even at worst-case
        # inputs, so a score below 50 already reflects a severely unhealthy
        # instance - a threshold under 30 is mathematically unreachable.
        assert 0.0 <= score < 50.0  # Should be unhealthy
...
class TestLoadBalancingMetrics:
    ...
    def test_should_accept_request_low_health_score(self):
        ...
        CPU/memory are kept at or below 90 so the health-score check (which
        runs first) is what actually rejects the request, rather than the
        CPU-specific or memory-specific checks that run right after it.
        """
        metrics = LoadBalancingMetrics()

        # Mock a moderately-but-broadly degraded instance (no single metric
        # extreme enough to trip the CPU/memory-specific checks).
        with patch.object(metrics, "get_instance_load") as mock_load:
            mock_load.return_value = InstanceLoad(
                instance_id="test",
                active_requests=100,
